...
'''
    ...
    def dl_eggnog_db(self):
        ...
        cmd = ['wget -c', URL.EGGNOG_DB_URL, '-O', eggnog_db_gz, '&& gunzip', eggnog_db_gz, '\n',
               'wget -c', URL.EGGNOG_DMND_URL, '-O', eggnog_dmnd_gz, '&& gunzip', eggnog_dmnd_gz, '\n']
        return cmd
    def dl_kofamscan_db(self):
        ...
        cmd = ['wget -c', URL.KO_LIST_URL, '-O', ko_list_gz, '&& gunzip', ko_list_gz, '\n',
               'wget -c', URL.KO_PROFILES_URL, '-O', profiles_tgz, '&& tar xzf', profiles_tgz, '-C', wkdir, '\n']
        return cmd
    def dl_dramv_db(self):
        wkdir = f'{self.outdir}/VC-DRAMvDB'
        utils.mkdir(wkdir)
        cmd = ['conda run -n VC-DRAMv DRAM-setup.py prepare_databases', '--skip_uniref', '--output_dir', wkdir, '\n']
        return cmd
    def dl_gtdbtk_db(self):
        ...
        cmd = ['wget -c', URL.GTDBTK_DB_URL, '-O', gtdbtk_db_gz, '--no-check-certificate','&& tar xzf',gtdbtk_db_gz,'-C',wkdir,'\n',
               'mv', db_virsion_dir, dbdir, '&& rm -f', gtdbtk_db_gz, '\n']
