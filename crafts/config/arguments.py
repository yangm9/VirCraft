...
#---------------------binning------------------------
    ...
    subpsr.add_argument(
        '-b', '--coverage-files', action='store', type=str, nargs='+', dest='coverage_files', metavar='STR', default=False, required=True,
...
def addFaArg(psr, seq_type='vctg'):
    HelpDict = {
        'ctg': 'Path to a FastA file with metagenomic sequences. e.g., spades_assembled_contigs.fsa',
        'vctg': 'Path to a FastA file with viral configs or vOTUs sequences. e.g., viral_positive_contigs.fsa',
        'gene': 'Path to a FastA file with gene sequences. e.g., viral_positive_contigs.ffn',
        'prot': 'Path to a FastA file with protein sequences. e.g., viral_positive_contigs.faa'
    }
    psr.add_argument(
...
def addCutoffArg(psr):
    psr.add_argument(
        '-l', '--min-len', action='store', type=int, dest='min_len', metavar='INT', default=2000,
...
def addTaxaArg(psr):
    psr.add_argument(
        ...
        help='Viral taxonomic annotation table, which generated by classify module in VirCraft packages. [default=None]'
    )
    return psr

def addAbdTabArg(psr):
    psr.add_argument(
        '-i', '--abundance_table', action='store', type=str, dest='abundance_table', metavar='STR', default=None,
        help='vOTU/gene abundance table, if provided, skip abundance calculation steps. [default=None]'
    )
    return psr
