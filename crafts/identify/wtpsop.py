...
class posiWTP(VirSeq):
    ...
    def wtp(self):
        ...
        utils.mkdir(wkdir)
        cmd = ['nextflow run ~/.nextflow/assets/replikation/What_the_Phage', '--fasta', self.fasta, '--databases', self.confDict['wtpDB'],
            '--cachedir', self.confDict['wtpIMG'], '--output', resdir, '--workdir', tmpdir, '--cores', self.threads, '-profile local,singularity --filter 10000\n']
        return cmd, resdir
    def curate(self):
        ...
        cmd.extend(
            ...
            'filtByViralPredictionTools.py', wtpdir,checkvdir, '>', posi_ctg_list, '\n',
